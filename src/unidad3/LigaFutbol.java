...
public class LigaFutbol {
	 ...
	 * posiciones equipos, si posiciones es 0 Mostrará todos
	 * 
	 * @param posiciones
	 */
	...
	/**
	 * La funcion recorre todos los equipos y dentro de cada equipo todos los
	...
	/**
	 * Equipos Descenso: Devuelve array equipos que descienden, siempre descienden 3
	 * equipos. primero serán los que tengan menos de 6 millones, si hay hueco libre
	 * despues los que no hayan marcado más de 4 goles, y si no los tres últimos.
	 * 
	 * @return
	 */
	// PORHACER
	public Equipo[] enDescenso() {
		...
		ordenarEquipos(listaEquipos);
		for (int i = 0; i < this.listaEquipos.length; i++) {
			if (posDes <= 3) {
				if (this.listaEquipos[i].getPresupuesto() < 6000000) {
					equiposDescenso[posDes] = this.listaEquipos[i];
					posDes++;
				} else if (this.listaEquipos[i].getNumGoles() <= 4) {
					equiposDescenso[posDes] = this.listaEquipos[i];
					posDes++;
				} else if (this.listaEquipos[i].getPuntos() == listaEquipos[listaEquipos.length - posDes].getPuntos()) {
					equiposDescenso[posDes] = this.listaEquipos[i];
					posDes++;
				}
			}
